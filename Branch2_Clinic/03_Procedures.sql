CREATE PROCEDURE dbo.usp_InsertPatient_B2
...
GO
---2. Insert Encouter---
CREATE PROCEDURE dbo.usp_InsertNewEncounter_B2
    @patient_id     INT,
    @encounter_date DATE,
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM [PATIENT.2] WHERE patient_id = @patient_id)
    BEGIN
...
GO
---3. Insert Specimen---
CREATE PROCEDURE dbo.usp_CreateSpecimen_B2
    @encounter_id    INT,
    @patient_id      INT,
...
BEGIN
    ...
    -- Basic checks
    IF NOT EXISTS (SELECT 1 FROM [ENCOUNTER.2] WHERE encounter_id = @encounter_id AND patient_id = @patient_id)
    BEGIN
        RAISERROR('Encounter does not exist for this patient at Branch_2.', 16, 1);
        RETURN;
    END;
...
-- 4. Allow Clinic 2 to update specimen status (and push to Lab)
CREATE PROCEDURE dbo.usp_UpdateSpecimenStatus_B2
    @specimen_id INT,
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM [SPECIMEN.1.2] AS s
        WHERE s.specimen_id = @specimen_id;

        -- Update Lab fragment (linked server)
        UPDATE s2
        SET s2.status = @new_status
        FROM Branch_3.Branch_3.dbo.[SPECIMEN.2] AS s2
        WHERE s2.specimen_id = @specimen_id;
...
---5. Get Results---
CREATE PROCEDURE dbo.usp_GetEncounterTestResults_B2
    @encounter_id INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT
        ...
        tr.result_numeric
    FROM [ENCOUNTER.2] e
    JOIN [PATIENT.2]      p   ON p.patient_id   = e.patient_id
    JOIN [SPECIMEN.1.2]   s   ON s.encounter_id = e.encounter_id
    ...
    LEFT JOIN Branch_3.Branch_3.dbo.TEST_RESULT tr
          ...
          AND tr.branch_id   = 2        -- Site 2
    WHERE e.encounter_id = @encounter_id;
END;
GO

---6. Synchronize deletion ---
CREATE OR ALTER PROCEDURE dbo.usp_DeleteSpecimen_B2
    @specimen_id INT
AS
BEGIN
    SET NOCOUNT ON;
